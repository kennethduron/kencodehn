-- Refine safe-delete eligibility so auxiliary creation/edit history does not
-- masquerade as business history. Financial and commercial dependencies remain
-- authoritative blockers and all cleanup stays explicit.

create or replace function private.module_lifecycle_blockers(p_id uuid)
returns jsonb
language plpgsql
stable
security definer
...
begin
  if exists(
    ...
    join public.receivables r on r.id=pa.receivable_id
    left join public.add_on_installments i on i.id=r.add_on_installment_id
    left join public.add_on_payment_plans pp on pp.id=i.payment_plan_id
    left join public.add_on_recurring_services ars on ars.id=r.add_on_recurring_service_id
    left join public.add_on_sales s on s.id=coalesce(pp.add_on_sale_id,ars.add_on_sale_id)
    where pa.reversed_at is null
      and (s.add_on_id=p_id or r.metadata->>'addOnId'=p_id::text)
  ) then v_blockers:=v_blockers||jsonb_build_array('payment'); end if;

  if exists(select 1 from public.add_on_sales where add_on_id=p_id)
     or exists(select 1 from public.project_add_ons where id=p_id and (commercial_status='approved' or accepted_proposal_id is not null or coalesce(accepted_amount_minor,0)>0))
  then v_blockers:=v_blockers||jsonb_build_array('approved_sale'); end if;
  ...
  if exists(
    ...
    left join public.add_on_recurring_services ars on ars.id=r.add_on_recurring_service_id
    left join public.add_on_sales s on s.id=coalesce(pp.add_on_sale_id,ars.add_on_sale_id)
    where s.add_on_id=p_id or r.metadata->>'addOnId'=p_id::text
  ) then v_blockers:=v_blockers||jsonb_build_array('receivable'); end if;

  if exists(select 1 from public.add_on_proposals where add_on_id=p_id and status<>'draft')
  then v_blockers:=v_blockers||jsonb_build_array('commercial_proposal'); end if;

  if exists(select 1 from public.mail_threads where add_on_id=p_id)
  then v_blockers:=v_blockers||jsonb_build_array('commercial_mail'); end if;

  if exists(select 1 from public.mail_drafts where add_on_id=p_id)
  then v_blockers:=v_blockers||jsonb_build_array('mail_draft'); end if;

  -- These events are defensive blockers if an authoritative commercial row was
  -- removed outside the supported workflows. Pure creation, editing, assignment,
  -- and development-status events are intentionally auxiliary.
  if exists(
    select 1 from public.activity_logs
    where add_on_id=p_id and action in (
      'proposal_sent','proposal_rejected','module_sale_approved',
      'module_payment_plan_activated','module_recurring_charge_configured',
      'module_payment_received'
    )
  ) then v_blockers:=v_blockers||jsonb_build_array('commercial_audit'); end if;
...
begin
  if p_entity='lead' then return
    ...
    exists(select 1 from public.notifications where lead_id=p_id and type not in ('lead','lead_new')) or
    ...
    exists(select 1 from public.activity_logs where lead_id=p_id and action not in (
      'lead_created','lead_received','public_lead_created','lead_updated',
      'lead_priority_changed','lead_assigned','lead_reassigned','lead_unassigned'
    ));
    ...
    exists(select 1 from public.activity_logs where client_id=p_id and action not in (
      'client_created','client_updated','client_assigned','client_reassigned','client_unassigned'
    ));
    ...
    exists(select 1 from public.activity_logs where project_id=p_id and action not in (
      'project_created','project_updated','project_assigned','project_reassigned','project_unassigned'
    ));
  elsif p_entity='module' then return jsonb_array_length(private.module_lifecycle_blockers(p_id))>0;
    ...
    exists(select 1 from public.notifications where task_id=p_id and type not in ('task_created','task_updated')) or
    ...
    exists(select 1 from public.activity_logs where task_id=p_id and action not in (
...
declare
  v_role text; v_exists boolean; v_history boolean; v_delete boolean; v_archive boolean;
  v_reason text; v_recommended text; v_blockers jsonb:='[]'::jsonb;
begin
  ...
  if p_entity='module' then v_blockers:=private.module_lifecycle_blockers(p_id); end if;
  ...
  if p_entity in ('payment','receivable','expense') then
    v_delete:=false; v_archive:=false;
    v_recommended:=case p_entity when 'payment' then 'reverse' when 'receivable' then 'cancel' else 'reverse' end;
    ...
    v_reason:=case when p_entity='module'
      then 'Este módulo no tiene ventas, cobros ni pagos registrados y puede eliminarse definitivamente.'
      else 'Este registro no tiene actividad empresarial relacionada y puede eliminarse definitivamente.' end;
  elsif v_history then
    v_recommended:=case when p_entity in ('client','recurring_service','add_on_recurring','mail_identity') then 'deactivate' when p_entity in ('project','proposal','task') then 'cancel' else 'archive' end;
    if p_entity='module' then
      v_reason:=case
        when v_blockers ? 'payment' then 'No puede eliminarse porque tiene un pago registrado.'
        when v_blockers ? 'approved_sale' then 'No puede eliminarse porque existe una venta aprobada.'
        when v_blockers ? 'receivable' then 'No puede eliminarse porque tiene un cobro registrado.'
        when v_blockers ? 'commercial_mail' then 'No puede eliminarse porque contiene correspondencia comercial.'
        when v_blockers ? 'mail_draft' then 'No puede eliminarse porque tiene un borrador de correo vinculado.'
        when v_blockers ? 'commercial_proposal' then 'No puede eliminarse porque tiene una propuesta comercial que debe conservarse.'
        else 'No puede eliminarse porque contiene historial comercial que debe conservarse.' end;
    else v_reason:='Este registro tiene actividad y debe conservarse para mantener el historial de Ken Code.'; end if;
  else
    v_recommended:='none'; v_reason:='Su rol no permite una acción destructiva sobre este registro.';
  end if;
  return jsonb_build_object(
    'entity',p_entity,'id',p_id,'exists',true,'hasHistory',v_history,
    'deleteAllowed',v_delete,'archiveAllowed',v_archive,
    'recommendedAction',v_recommended,'reason',v_reason,'blockingReasons',v_blockers,'role',v_role
  );
end;
$$;
...
begin
  ...
  if p_action='delete' then
    if p_entity='lead' then
      delete from public.notifications where lead_id=p_id;
      update public.activity_logs set lead_id=null where lead_id=p_id;
      delete from public.leads where id=p_id returning name into v_name;
    elsif p_entity='client' then
      delete from public.seller_assignment_events where client_id=p_id;
      update public.activity_logs set client_id=null where client_id=p_id;
      delete from public.clients where id=p_id returning coalesce(nullif(company,''),name) into v_name;
    elsif p_entity='project' then
      delete from public.seller_assignment_events where project_id=p_id;
      update public.activity_logs set project_id=null where project_id=p_id;
      delete from public.projects where id=p_id returning name into v_name;
    elsif p_entity='module' then
      -- Only auxiliary rows can reach this branch; commercial/financial rows are
      -- rejected by record_lifecycle_inspect before any cleanup starts.
      delete from public.add_on_seller_assignment_events where add_on_id=p_id;
      delete from public.notifications where add_on_id=p_id;
      update public.activity_logs set add_on_proposal_id=null
        where add_on_proposal_id in (select id from public.add_on_proposals where add_on_id=p_id and status='draft');
      ...
      update public.activity_logs set add_on_proposal_id=null where add_on_proposal_id=p_id;
      delete from public.add_on_proposals where id=p_id returning title into v_name;
    elsif p_entity='task' then
      delete from public.notifications where task_id=p_id;
      update public.activity_logs set task_id=null where task_id=p_id;
      delete from public.tasks where id=p_id returning title into v_name;
...
revoke all on function private.module_lifecycle_blockers(uuid) from public, anon, authenticated;
revoke all on function private.record_lifecycle_has_history(text,uuid) from public, anon, authenticated;
...
comment on function private.module_lifecycle_blockers(uuid) is
  'Returns authoritative commercial and financial blockers. Drafts, assignment events, work status, and auxiliary audit rows do not block safe deletion by themselves.';
