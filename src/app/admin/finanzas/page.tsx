...
export default async function FinancePage({searchParams}:{searchParams:Promise<Record<string,string|string[]|undefined>>}){const admin=await getCurrentAdmin();if(!admin)return <AdminLogin authProvider={getCrmAuthProvider()} missingServerEnv={getMissingAdminEnv()} missingClientEnv={getMissingAuthClientEnv()}/>;if(!hasPermission(admin,"finance:view"))redirect("/admin");const p=await searchParams;const value=(key:string)=>typeof p[key]==="string"?p[key] as string:undefined;const range=resolveFinanceRange({period:value("period"),from:value("from"),to:value("to")});const[summary,movements,series,moduleSummary,context]=await Promise.all([financeSummary(range.from,range.to),financeReport({report:"cash_result",from:range.from,to:range.to,currency:"USD",pageSize:6}),financeSeries(range.from,range.to),financeAddOnSummary(range.from,range.to),commercialPageContext(admin)]);return <AdminChrome admin={admin} unreadCount={context.unreadCount} authProvider={getCrmAuthProvider()}><FinanceDashboard summary={summary} series={{USD:series}} movements={movements.items} range={range} moduleSummary={moduleSummary}/></AdminChrome>}
