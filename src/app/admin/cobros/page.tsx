...
export default async function BillingPage({searchParams}:{searchParams:Promise<Record<string,string|string[]|undefined>>}){const admin=await getCurrentAdmin();if(!admin)return <AdminLogin authProvider={getCrmAuthProvider()} missingServerEnv={getMissingAdminEnv()} missingClientEnv={getMissingAuthClientEnv()}/>;if(!hasPermission(admin,"billing:view"))redirect("/admin");const p=await searchParams;const value=(key:string)=>typeof p[key]==="string"?p[key] as string:undefined;const[receivables,summary,context]=await Promise.all([listReceivables({page:Number(value("page")||1),state:value("state"),timing:value("timing"),origin:value("origin"),currency:value("currency")?.toUpperCase()}),billingSummary(),commercialPageContext(admin)]);return <AdminChrome admin={admin} unreadCount={context.unreadCount} authProvider={getCrmAuthProvider()}><div className="grid gap-6"><div><p className="text-sm font-black uppercase tracking-[.2em] text-kc-cyan">Cobros</p><h1 className="mt-2 font-display text-3xl font-black sm:text-4xl">Cuentas por cobrar</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-kc-muted">Obligaciones reales, pagos aplicados y saldos por moneda. Los acuerdos y mensualidades permanecen separados.</p></div><BillingPanel {...receivables} summary={summary as any} canManage={hasPermission(admin,"payments:manage")}/></div></AdminChrome>}
