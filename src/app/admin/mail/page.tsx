...
export default async function MailPage({ searchParams }: { searchParams: Promise<{ folder?: string; q?: string; thread?: string; compose?: string; to?: string; lead?: string; client?: string; project?: string; addOn?: string; proposal?: string }> }) {
  ...
  if (params.compose === "1") { const client = await createSupabaseServerClient(); const [lead, company, project, addOn, proposal] = await Promise.all([params.lead ? client.from("leads").select("name,company").eq("id", params.lead).maybeSingle() : null, params.client ? client.from("clients").select("name,company").eq("id", params.client).maybeSingle() : null, params.project ? client.from("projects").select("name").eq("id", params.project).maybeSingle() : null, params.addOn ? client.from("project_add_ons").select("name").eq("id", params.addOn).maybeSingle() : null, params.proposal ? client.from("add_on_proposals").select("proposal_number").eq("id", params.proposal).maybeSingle() : null]); const person = company?.data || lead?.data; context.clientName = person?.name || person?.company || ""; context.businessName = person?.company || ""; context.projectName = project?.data?.name || ""; context.moduleName = addOn?.data?.name || ""; context.proposalNumber = proposal?.data?.proposal_number || ""; }
