...
export default async function MailPage({ searchParams }: { searchParams: Promise<{ folder?: string; q?: string; cursor?: string; thread?: string; compose?: string; to?: string; lead?: string; client?: string; project?: string; addOn?: string; proposal?: string }> }) {
  ...
  return <AdminChrome admin={admin} authProvider={getCrmAuthProvider()}><MailWorkspace admin={admin} initial={{ ...initial, selected }} composeContext={{ open: params.compose === "1", to: params.to || "", leadId: params.lead, clientId: params.client, projectId: params.project, addOnId: params.addOn, proposalId: params.proposal, ...context }} readOnlyPreview={isCrmPreviewReadOnly()} /></AdminChrome>;
}
