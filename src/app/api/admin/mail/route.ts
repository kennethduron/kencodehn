  ...
  .object({
    ...
    context: z
      .object({
        ...
        addOnId: uuidSchema.nullable().optional(),
...
const deleteDraftSchema = z
  .object({ action: z.literal("delete_draft"), draftId: uuidSchema })
  .strict();
const stateSchema = z
  .object({
    action: z.enum([
...
function mailError(reason: unknown) {
  ...
  return NextResponse.json(
    { error: "No pudimos completar la operación de correo." },
    { status: 500 },
  );
}
...
export async function GET(request: NextRequest) {
  ...
  const folder = folderSchema.safeParse(
    request.nextUrl.searchParams.get("folder") || "inbox",
  );
  ...
  try {
    ...
    const thread =
      threadId && uuidSchema.safeParse(threadId).success
        ? await loadThread(auth.admin, threadId)
        : null;
    const draft =
      draftId && uuidSchema.safeParse(draftId).success
        ? await loadDraft(auth.admin, draftId)
        : null;
    return NextResponse.json({
      ...list,
      selected: thread,
...
export async function POST(request: NextRequest) {
  ...
  if (sent.success) {
    try {
      return NextResponse.json({
        ok: true,
        ...(await sendMail(auth.admin, sent.data)),
      });
    } catch (error) {
      return mailError(error);
    }
  ...
  if (draft.success) {
    ...
    const cleanHtml = sanitizeMailHtml(draft.data.html);
    ...
    const values = {
      ...
      body_html: cleanHtml,
      body_text: textFromHtml(cleanHtml),
      lead_id: draft.data.context.leadId || null,
      client_id: draft.data.context.clientId || null,
      project_id: draft.data.context.projectId || null,
      add_on_id: draft.data.context.addOnId || null,
      proposal_id: draft.data.context.proposalId || null,
      updated_at: now,
    };
    if (draft.data.id) {
      const nextVersion = (draft.data.version || 1) + 1;
      const result = await client
        .from("mail_drafts")
        .update({ ...values, version: nextVersion })
        .eq("id", draft.data.id)
        .eq("owner_id", auth.admin.uid)
        .eq("version", draft.data.version || 1)
      ...
      if (!result.data)
        return NextResponse.json(
          {
            error:
              "El borrador cambió en otra sesión. Recargue antes de sobrescribirlo.",
          },
          { status: 409 },
        );
      await client
  ...
  if (deletedDraft.success) {
    ...
    const owned = await client
      ...
      .eq("owner_id", auth.admin.uid)
      .maybeSingle();
    if (owned.error) return mailError(owned.error);
    if (!owned.data)
      return NextResponse.json(
    ...
    const files = await client
      ...
      .select("storage_path")
      .eq("draft_id", deletedDraft.data.draftId);
    if (files.error) return mailError(files.error);
    const paths = (files.data || []).map((item) => item.storage_path);
    if (paths.length) {
  ...
  if (state.success) {
    if (!(await mayAccessThread(auth.admin, state.data.threadId)))
    ...
    if (state.data.action === "read" || state.data.action === "unread")
      ...
      const update =
        state.data.action === "important"
          ? { is_important: state.data.value ?? true, updated_at: now }
          : {
              state:
                state.data.action === "archive"
                  ? "archived"
                  : state.data.action === "trash"
                    ? "trash"
                    : "inbox",
              updated_at: now,
            };
  ...
  if (assign.success) {
    if (
      !hasPermission(auth.admin, "mail:assign_threads") ||
      !(await mayAccessThread(auth.admin, assign.data.threadId))
    )
      return NextResponse.json({ error: "No autorizado." }, { status: 403 });
    const client = createSupabaseAdminClient();
    if (assign.data.profileId) {
      const target = await client
        .from("profiles")
        .select("id")
        .eq("id", assign.data.profileId)
        .eq("active", true)
    ...
    await client
      .from("mail_threads")
      .update({
        assigned_to: assign.data.profileId,
  ...
  if (follow.success) {
    if (!(await mayAccessThread(auth.admin, follow.data.threadId)))
      return NextResponse.json({ error: "No autorizado." }, { status: 403 });
    const client = createSupabaseAdminClient();
    const now = new Date().toISOString();
  ...
  return NextResponse.json(
    { error: "Solicitud de correo inválida." },
    { status: 400 },
  );
}
