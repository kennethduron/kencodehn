...
export function ClientDetail({ client, projects, tasks, activity, assignments, billingReceivables, billingPayments, members, canEdit, canAssign, canManageBilling }: {
  ...
  return <div className="grid gap-5">
    ...
    {tab === "communications" ? <div className="kc-admin-card grid min-h-64 place-items-center p-8 text-center"><div><Mail className="mx-auto text-blue-700" size={32} /><h2 className="mt-4 font-display text-2xl font-black text-kc-text">Comunicaciones del cliente</h2><p className="mt-2 max-w-lg text-sm leading-6 text-kc-muted">Redacte desde una identidad corporativa. La conversación quedará vinculada a esta ficha.</p>{client.email ? <Link href={`/admin/mail?compose=1&to=${encodeURIComponent(client.email)}&client=${client.id}`} className="mt-5 inline-flex min-h-11 items-center gap-2 rounded-xl bg-blue-700 px-4 text-sm font-black text-white"><Mail size={17} /> Redactar correo</Link> : <p className="mt-4 text-sm font-bold text-amber-700">Agregue un correo al cliente para iniciar una conversación.</p>}<Link href={`/admin/mail?q=${encodeURIComponent(client.email || client.name)}`} className="mt-3 block text-sm font-bold text-blue-700">Buscar conversaciones</Link></div></div> : null}
