...
import type { ProfileField } from "@/lib/admin/profile-validation";
...
export function ProfilePanel({ admin }: { admin: AdminUser }) {
  ...
  function updateField<K extends ProfileField>(field: K, value: Profile[K]) {
    setProfile((current) => ({ ...current, [field]: value }));
    setFieldErrors((current) => ({ ...current, [field]: undefined }));
  }
  async function save(event: FormEvent) {
    event.preventDefault(); setSaving(true); setError(""); setMessage(""); setFieldErrors({});
    const payload = { displayName: profile.displayName, preferredName: profile.preferredName, jobTitle: profile.jobTitle, phone: profile.phone, locale: profile.locale };
    const response = await fetch("/api/admin/profile", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
    const body = await response.json(); setSaving(false);
    if (!response.ok) { setFieldErrors(body.fieldErrors || {}); return setError(body.error || "No pudimos guardar el perfil."); }
    if (body.profile) setProfile((current) => ({ ...current, ...body.profile }));
  ...
  return <div className="grid gap-5 xl:grid-cols-[18rem_minmax(0,1fr)]"><Toast message={error || message} variant={error ? "error" : "success"} />
    <section className="kc-admin-card self-start rounded-2xl border p-5 text-center"><div className="mx-auto grid h-28 w-28 place-items-center overflow-hidden rounded-full bg-blue-50 text-3xl font-black text-blue-700 ring-4 ring-white shadow-lg">{profile.hasPhoto ? <img key={photoVersion} src={`/api/admin/profile/photo?v=${photoVersion}`} alt="Foto de perfil" className="h-full w-full object-cover" /> : initials}</div><h2 className="mt-4 break-words font-display text-xl font-black">{profile.displayName || "Perfil"}</h2><p className="mt-1 break-all text-sm text-kc-muted">{profile.email || admin.email}</p><label className="mt-5 inline-flex min-h-11 cursor-pointer items-center justify-center gap-2 rounded-xl bg-blue-700 px-4 text-sm font-black text-white hover:bg-blue-800"><Camera size={17} /> Cambiar foto<input type="file" accept="image/jpeg,image/png,image/webp" onChange={uploadPhoto} className="sr-only" disabled={saving} /></label>{profile.hasPhoto ? <button type="button" onClick={removePhoto} disabled={saving} className="mt-2 inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-xl text-sm font-bold text-rose-700 hover:bg-rose-50"><Trash2 size={16} /> Quitar foto</button> : null}<p className="mt-3 text-xs leading-5 text-kc-muted">JPG, PNG o WebP. Se reduce antes de guardarse de forma privada.</p></section>
    <form onSubmit={save} className="kc-admin-card min-w-0 rounded-2xl border p-5 sm:p-6"><div className="flex items-start gap-3"><span className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-blue-50 text-blue-700"><UserRound size={20} /></span><div><h2 className="font-display text-xl font-black">Información personal</h2><p className="mt-1 text-sm text-kc-muted">Estos datos no cambian su rol ni sus permisos.</p></div></div><div className="mt-6 grid gap-4 md:grid-cols-2">
      <label className="grid gap-1.5 text-sm font-bold">Nombre completo<input value={profile.displayName} onChange={(e) => updateField("displayName", e.target.value)} maxLength={160} autoComplete="name" aria-invalid={Boolean(fieldErrors.displayName)} aria-describedby={fieldErrors.displayName ? "profile-display-name-error" : undefined} className="min-h-11 rounded-xl border px-3" />{fieldErrors.displayName ? <span id="profile-display-name-error" className="text-xs font-semibold text-rose-700">{fieldErrors.displayName}</span> : null}</label>
      <label className="grid gap-1.5 text-sm font-bold">Nombre preferido<input value={profile.preferredName} onChange={(e) => updateField("preferredName", e.target.value)} maxLength={100} aria-invalid={Boolean(fieldErrors.preferredName)} aria-describedby={fieldErrors.preferredName ? "profile-preferred-name-error" : undefined} className="min-h-11 rounded-xl border px-3" />{fieldErrors.preferredName ? <span id="profile-preferred-name-error" className="text-xs font-semibold text-rose-700">{fieldErrors.preferredName}</span> : null}</label>
      <label className="grid gap-1.5 text-sm font-bold">Cargo<input value={profile.jobTitle} onChange={(e) => updateField("jobTitle", e.target.value)} maxLength={140} autoComplete="organization-title" aria-invalid={Boolean(fieldErrors.jobTitle)} aria-describedby={fieldErrors.jobTitle ? "profile-job-title-error" : undefined} className="min-h-11 rounded-xl border px-3" />{fieldErrors.jobTitle ? <span id="profile-job-title-error" className="text-xs font-semibold text-rose-700">{fieldErrors.jobTitle}</span> : null}</label>
      <label className="grid gap-1.5 text-sm font-bold">Teléfono (opcional)<input value={profile.phone} onChange={(e) => updateField("phone", e.target.value)} maxLength={60} autoComplete="tel" inputMode="tel" aria-invalid={Boolean(fieldErrors.phone)} aria-describedby={fieldErrors.phone ? "profile-phone-error" : undefined} className="min-h-11 rounded-xl border px-3" />{fieldErrors.phone ? <span id="profile-phone-error" className="text-xs font-semibold text-rose-700">{fieldErrors.phone}</span> : null}</label>
      <label className="grid gap-1.5 text-sm font-bold">Idioma<select value={profile.locale} onChange={(e) => updateField("locale", e.target.value as Profile["locale"])} aria-invalid={Boolean(fieldErrors.locale)} aria-describedby={fieldErrors.locale ? "profile-locale-error" : undefined} className="min-h-11 rounded-xl border px-3"><option value="es-HN">Español</option><option value="en-US">English</option></select>{fieldErrors.locale ? <span id="profile-locale-error" className="text-xs font-semibold text-rose-700">{fieldErrors.locale}</span> : null}</label>
